...
The EVALUATE `reviewer` stance is a mechanical conformance/completeness pass. It runs **first** in Phase 3 Review, as a gate before the `red-team`. It is distinct from the `red-team`: bdplan deliberately carries both stances (the asymmetry vs bdresearch, which has only `red-team`, is justified by the factoring test — semantic plan conformance warrants a dedicated pass).
