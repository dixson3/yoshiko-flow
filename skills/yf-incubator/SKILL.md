...
[ -n "$SKILL_DIR" ] || SKILL_DIR="$(yf skill-dir yf-incubator 2>/dev/null)"
...
if [ -z "$SKILL_DIR" ]; then
  ...
  do
    if [ -d "$_root/yf-incubator" ]; then SKILL_DIR="$_root/yf-incubator"; break; fi
  done
  unset _root
fi

[ -z "$SKILL_DIR" ] && { echo "ERROR: yf-incubator skill directory not found" >&2; exit 1; }
```
