...
Skills are **`rust-embed`-baked into the `yf` binary at build time** (`yf/src/embed.rs`
carries `#[folder = "../skills"]`), then deployed by `yf skills install`. Consequences:

- Editing `skills/yf-plan/` does **not** hot-swap the running skill. The bytes the
  resolver loads were frozen into the `yf` binary at its last `cargo build`.
- To exercise a repo edit you must **rebuild** (`cargo build`, re-embeds `../skills`)
  and **re-install** (`yf skills install`) — then run the skill.
...
| File | Purpose |
| --- | --- |
| `bootstrap.sh` | Build+install the MODIFIED skill into an isolated sandbox HOME; assert the resolver's first hit is the sandbox copy. |
