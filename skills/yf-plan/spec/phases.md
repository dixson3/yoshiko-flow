...
**Amendment (plan-068 Issue 0.6, dixson3/yoshiko-flow#331): the named-branch model applies to a branch cut with NO `worktree add`.** Under `execute.worktree: false` no worktree is created, but `<plan-id>-execute` is still the branch execution happens on and still the branch `land` merges. The name is a property of the **phase**, not of the mechanism that materialises it. So `_worktree_ensure` shall create and check out `<plan-id>-execute` **in the primary checkout** on the in-place path, and every derivation of the name (`_worktree_path`, `_plan_id_from_dir`, `_worktree_teardown`, `_land_manifest`'s branch-existence check) is unchanged and applies unchanged. Measured: `_land_manifest` halts `execute-branch-missing` on a bare `git rev-parse --verify <plan-id>-execute` and **never calls `_worktree_opted_out()`** — the manifest has no concept of in-place mode, so it is the *absent branch*, not the absent worktree, that made the landing route unreachable. **Two** consecutive plans (062, 063) hand-cut that branch as an explicit numbered issue in their own Epic 0, and plan-068 makes three. That is accumulating residue, not an edge case.
...
**Amendment (plan-068 Issue 0.6, dixson3/yoshiko-flow#331), two clauses.**

**(i) The pinned base binds an in-place `checkout -b` exactly as it binds `worktree add -b`.** The requirement is about the *start-point*, not about the command. In-place, `_worktree_ensure` shall pass the strategy-resolved base explicitly — `git checkout -b <plan-id>-execute <pinned-base>` — never relying on ambient HEAD. This is load-bearing for a **structural** reason: today the `opted-out` short-circuit sits at the **top** of `_worktree_ensure`, *before* `_worktree_viability`, `_resolve_execute_base` and the bd-resolution probe, so the in-place path never reaches the base resolver at all. Satisfying this clause requires reaching it.

The create must be a **three-way branch**, because `_worktree_ensure` is called on **every** `execute` invocation and its own docstring promises "idempotent create-or-reattach", while `git checkout -b` on an existing branch exits **128**: branch absent → `checkout -b <b> <pinned-base>`; branch exists → plain `checkout <b>`; already on it → no-op. **Create-without-checkout is silently wrong** — the `checkout` half is not optional, because the whole point is that `ctx.root`'s HEAD *is* the execute branch (`REQ-LAND-002` as amended).

**(ii) THE DIRTY-TREE REFUSAL CLASS.** The in-place branch cut shall **refuse, with a declared refusal class**, when the working tree is dirty in a way that would make the checkout unsafe — rather than letting `git` fail and reporting the raw error. Measured: `git checkout -b` on a dirty divergent tree emits `error: Your local changes would be overwritten by checkout`, exits **1**, and leaves HEAD unmoved. `_worktree_ensure` has **no guard** for that class today, so the failure surfaces as an unhandled `git` error at the moment execution begins.

The refusal is a **clause of this requirement and carries no new REQ id.** It governs the same operation this requirement already governs — cutting `<plan-id>-execute` from a pinned base — so it is a property of that operation, not a separate one. (plan-068's `assets/req-allocation.md` records the decision and its second reason: the plan's one-shot capability gate checks the alternation `REQ-LAND-(037|038)` and resolves before this text lands, so an id allocated here could never have been checked by it.)
...
**Amendment (plan-068 Issue 0.6, dixson3/yoshiko-flow#331): AN EXPLICIT IN-PLACE CARVE-OUT.** "Between phases the primary checkout is restored to a known branch (never left on a plan branch)" does **not** hold **for the duration of in-place execution**, and the in-place design contradicts it deliberately rather than incidentally.

Under `execute.worktree: false` there is one address space. Execution must happen **on** `<plan-id>-execute` — that is what makes `ctx.root`'s HEAD the execute branch, what makes L1's down-merge real rather than a self-merge, and what keeps a commit from landing on the merge target and escaping the merge L3 validates. So the primary checkout **is** left on a plan branch, for exactly as long as the plan is `executing`.

**The carve-out is bounded, and the bound is what keeps the original rationale intact.** REQ-BRANCH-004 exists because "leaving the primary on a plan branch reintroduces ambient-HEAD drift for the next plan". In-place, that risk is discharged at **L2**, which checks out the merge target, and at **L18**, whose teardown deletes `<plan-id>-execute` after the merge. The carve-out covers the interval **between the in-place branch cut and L2**; outside that interval the requirement applies unchanged. Teardown's per-branch guard is unchanged: `<plan-id>-execute` only, and a feature `<plan-id>` branch is still preserved under the `feature-branch` strategy.

Recorded explicitly rather than left as a silent exception: a design that contradicts a requirement and says nothing is indistinguishable from a design that violates it.
