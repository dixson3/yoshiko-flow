...
REQ-AGENT-066: *(added plan-071 Issue 0.1 / #323, #338, #286, #306, #390)* **The execution-pass contract.** Every red-team pass carries a **mode**, keyed on its **pass index** — the count of `reviews/pass-*.md` files that exist when the pass is dispatched, plus one. Passes **1 and 2 may be reading passes**; pass **3 and every later pass shall be an execution pass**. The pass emits `**Mode:** reading` or `**Mode:** execution` on the line directly under the `## Verdict:` heading of `pass-N.md`, so the mode is a parseable field rather than a claim in prose.

An execution pass **shall run**, and record the exit code of, every shipped bundle checker over the plan — `doc_lint.py --path plan.md`, `plan_extract.py --strict`, `gate_consistency.py`, `check_amendment_log.py --plan <id>`, `check-req-coverage.py`, `okf.py reindex --check`, `plan_manager.py audit` — and every clause-form Success Criteria command under a 60-second bound; and it **shall re-verify every `resolved` cell** in every prior pass's Resolutions table by running the evidence that cell names (#306, the phantom resolution). A reading pass may do any of this; an execution pass must.

**The finding vocabulary is closed.** Every finding of an execution pass carries a `measured | inferred` column. A `measured:` finding cites the command it ran and the exit code it observed. An `inferred:` finding is a reading claim: it **cannot carry severity `high`** and cannot block approval (#390 — an inference recorded as a measurement survived six passes). **An execution pass with zero measured findings returns `APPROVE`.** That is the convergence standard (#286): a reading pass has no terminating rule, which is how an open brief on a converged plan manufactures concerns.
Rationale: measured over plans 062/063/068, every landing-chain defect was found by *running* something and none by the 27 reading passes; plan-068 pass 4 recorded that a further reading pass had negative value. Reading has near-zero recall on the silent-green class and execution has near-total recall, so the loop's later passes are converted from the instrument that cannot see the defect to the one that can. Two reading passes are kept because feasibility, risk and premise checks are reading judgements (REQ-AGENT-046..048) and the first two passes are where they pay. Honesty clause, as REQ-AGENT-049: this requirement constrains the brief and the file shape; that a pass genuinely ran its commands has no exit code beyond the ones it records.
Verification: `uv run skills/yf-plan/scripts/test_review_agent_contract.py`
