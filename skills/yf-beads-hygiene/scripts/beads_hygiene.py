...
# The active-set classifier constants (GATE_TYPE, CLOSED_STATUSES, PARENT_CHILD,
# IN_PROGRESS, OPEN, ACTIVE_*) now live in the generated active-set classifier region
# below (canonical: _shared/active_set.py, vendored in-place by _shared/sync.py).
...
# The active-set classifier (Edge, ActiveSetReport, classify_active and the constants/helpers
# they depend on) is the CANONICAL copy, vendored from _shared/active_set.py and regenerated
# in-place by _shared/sync.py. The audit axis (AuditReport, classify_edges) below is
# hygiene-specific and consumes Edge. Edge.classify returns the four class STRINGS, which equal
# the TRUE_ORPHAN/TRULY_DANGLING/SATISFIED_GATE/LIVE_GATE constants the audit axis keys on.
