...
`formulas/yf-research.formula.toml`, SKILL.md Phases 1–4, and the agent files.
...
REQ-PHASE-002: GATE is a human gate. In `quick` mode it is auto-resolved inline; otherwise it is resolved in a new session via `/yf-research coordinate`.
...
REQ-PHASE-007: A `type = "gate"` formula step yields a task wrapper (`yf-research.gate`) plus the real gate (`yf-research.gate-gate`); the gate is resolved via the `gate-*` key, while downstream `needs` depends on the wrapper.
Rationale: bd 1.0.5 compiles a gate step into two beads; resolving the wrapper key fails with `not a gate issue (type=task)`.
Verification: SKILL.md Phase 3 `GATE_ID = jq '.id_mapping["yf-research.gate-gate"]'`; see `beads-authoring` → Formula gate steps.
