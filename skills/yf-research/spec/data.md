...
REQ-DATA-006: Per the Skill Surface Convention, operator config (`ignore-skill`) lives at `.yf-research.local.json` (repo root, gitignored); runtime state (`prereqs-present` and `scaffold-ensured` caches) lives at `.yf/yf-research/preflight.json`; the installed rule (in the scope+surface rules dir, e.g. `~/.<surface>/rules/RESEARCH.md`, installed by `install.sh`) is hash-checked against `protocols/manifest.json` (schema_version 1). The anchored `.gitignore` entries `/.yf-research.local.json` and `/.state/` (no globs) are ensured by preflight (`_ensure_scaffold`), not by `/yf-research init` — additive-only, gated by `scaffold-ensured` so they are written once per scaffold version (Surface Convention §7).
