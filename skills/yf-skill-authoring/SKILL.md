...
[ -n "$SKILL_DIR" ] || SKILL_DIR="$(yf skill-dir yf-skill-authoring 2>/dev/null)"
...
if [ -z "$SKILL_DIR" ]; then
  ...
  do
    if [ -d "$_root/yf-skill-authoring" ]; then SKILL_DIR="$_root/yf-skill-authoring"; break; fi
  done
  unset _root
fi

[ -z "$SKILL_DIR" ] && { echo "ERROR: yf-skill-authoring skill directory not found" >&2; exit 1; }
```
...
1. **Companion rules.** Source: `${SKILL_DIR}/protocols/<NAME>.md`. Deployed by `yf harness skills install`, not `<skill> init`, to a rules dir anchored by install scope and harness — `--scope user` → `~/.<surface>/rules/<NAME>.md`, `--scope project` → `<git-root>/.<surface>/rules/<NAME>.md` (`--harness claude-code|codex|opencode|pi|agents`). Never write to `AGENTS/`. Never edit `CLAUDE.md`.
