...
4. **Local state.** `.yf/<short>/` (e.g. `.yf/<short>/preflight.json`), same short name as config. Skill scripts write runtime cache here only. Never under the skill source dir. Never under `.{claude,agents}/`. `yf-plan`'s `plan_manager.py` now conforms (`SKILL_SHORT = "plan"` → `.yf/plan/`); the former full-name drift tracked in `dixson3/yoshiko-flow#100` is resolved and that issue is closed.
