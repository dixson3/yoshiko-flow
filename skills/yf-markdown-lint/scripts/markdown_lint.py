...
Rules:
  ...
  ML009  Embedded renderable-fence source does not compile (opt-in shell-out, e.g. ``d2``)
  ML010  Bare CriticMarkup construct in prose (``{++ ++}`` / ``{-- --}`` / ``{~~ ~> ~~}`` /
         ``{== ==}`` / ``{>> <<}``)
  ML011  Image with empty alt text ``![](x.png)`` (a11y; a present title never warns)
...
# CriticMarkup delimiter registry (ML010). Documented and extensible: to add a
# construct, add a row with its ``open``/``close`` delimiters and an optional
# ``sep`` that MUST appear between them for the construct to fire. The
# substitution construct requires the ``~>`` separator (``{~~ old ~> new ~~}``);
# the other four fire on the delimiter pair alone.
CRITICMARKUP = {
    "addition":     {"open": "{++", "close": "++}", "sep": None},
    "deletion":     {"open": "{--", "close": "--}", "sep": None},
    "substitution": {"open": "{~~", "close": "~~}", "sep": "~>"},
    "highlight":    {"open": "{==", "close": "==}", "sep": None},
    "comment":      {"open": "{>>", "close": "<<}", "sep": None},
}
CRITICMARKUP_RE = {
    name: re.compile(re.escape(meta["open"]) + r"(.*?)" + re.escape(meta["close"]))
    for name, meta in CRITICMARKUP.items()
}
...
def split_dest_title(dest: str) -> tuple[str, str | None]:
    """Split a link/image destination into ``(path, title)``.

    GFM allows an optional **title** after the destination, separated by
    whitespace and wrapped in ``"..."``, ``'...'``, or ``(...)``. The path may
    itself be angle-bracket wrapped (``<path>``). Returns the bare path (angle
    brackets and any trailing title removed) plus the raw title text, or None
    when no title is present. This is what lets ML003 resolve
    ``![alt](path "caption")`` on ``path`` only (REQ-MDLINT-002a).
    """
    s = dest.strip()
    # angle-bracket wrapped path: <path> optionally followed by a title
    if s.startswith("<"):
        end = s.find(">")
        if end != -1:
            rest = s[end + 1:].strip()
            return s[1:end], (rest or None)
    m = re.search(r"""\s+("[^"]*"|'[^']*'|\([^)]*\))\s*$""", s)
    if m:
...
class Linter:
    ...
    def lint_file(self, path: Path) -> list[tuple[int, str, str]]:
        ...
        while i < n:
            ...
            # ML010 — bare CriticMarkup constructs in prose. Registry-driven
            # (five delimiter pairs); the substitution construct fires only when
            # its ``~>`` separator is present. Inline-code spans and fenced code
            # blocks are exempt — this runs on the inline-code-stripped ``code``
            # and never inside a fence (the fence body ``continue``s above).
            if "ML010" in self.rules:
                for name, rx in CRITICMARKUP_RE.items():
                    sep = CRITICMARKUP[name]["sep"]
                    for cm in rx.finditer(code):
                        if sep and sep not in cm.group(1):
                            continue
                        out.append((lineno, "ML010",
                                    f"bare CriticMarkup {name}: {cm.group(0)}"))

            # ML003 / ML004 / ML006 / ML011 markdown links & images
            for m in MDLINK_RE.finditer(code):
                whole = m.group(0)
                is_image = whole.startswith("!")
                alt = m.group("text")
                dest_raw = m.group("dest").strip()
                path_part, title = split_dest_title(dest_raw)

                # ML011 — image with empty alt text (a11y). Images only (never an
                # empty-text link); a present title never warns, blessing the
                # print-caption convention ``![alt](src "title")`` (REQ-MDLINT-007).
                if "ML011" in self.rules and is_image and not alt.strip() and title is None:
                    out.append((lineno, "ML011", f"image has empty alt text: {whole}"))

                if not dest_raw:
                    if "ML006" in self.rules:
                        out.append((lineno, "ML006", f"empty link destination: {whole}"))
                    continue
                # Resolve on the path only — the optional GFM title is stripped
                # ahead of path resolution (REQ-MDLINT-002a / #81).
                dest = path_part
                if not dest:
                    continue  # title-only destination, nothing to resolve
