...
[ -n "$SKILL_DIR" ] || SKILL_DIR="$(yf skill-dir yf-beads-authoring 2>/dev/null)"
...
if [ -z "$SKILL_DIR" ]; then
  ...
  do
    if [ -d "$_root/yf-beads-authoring" ]; then SKILL_DIR="$_root/yf-beads-authoring"; break; fi
  done
  unset _root
fi

[ -z "$SKILL_DIR" ] && { echo "ERROR: yf-beads-authoring skill directory not found" >&2; exit 1; }
```
