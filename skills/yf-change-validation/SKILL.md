...
[ -n "$SKILL_DIR" ] || SKILL_DIR="$(yf skill-dir yf-change-validation 2>/dev/null)"
...
if [ -z "$SKILL_DIR" ]; then
  ...
  do
    if [ -d "$_root/yf-change-validation" ]; then SKILL_DIR="$_root/yf-change-validation"; break; fi
  done
  unset _root
fi

[ -z "$SKILL_DIR" ] && { echo "ERROR: yf-change-validation skill directory not found" >&2; exit 1; }
```
