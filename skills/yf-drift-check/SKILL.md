...
[ -n "$SKILL_DIR" ] || SKILL_DIR="$(yf skill-dir yf-drift-check 2>/dev/null)"
...
if [ -z "$SKILL_DIR" ]; then
  ...
  do
    if [ -d "$_root/yf-drift-check" ]; then SKILL_DIR="$_root/yf-drift-check"; break; fi
  done
  unset _root
fi

[ -z "$SKILL_DIR" ] && { echo "ERROR: yf-drift-check skill directory not found" >&2; exit 1; }
```
