...
fn req_yf_mark_004_prune_removes_stray_keeps_embedded() {
    ...
    let extras = common::extra_deployed_files(SKILL, &skills_dir, "claude-code").unwrap();
    assert_eq!(
        extras,
...
// REQ-YF-MARK-005 (plan-044 Issue 2.11d, #155): each of the SEVEN residue paths
// exp-004 measured on the live machine is excluded from the tree hash.
//
// These are the exact additions that pinned `yf-plan` at `modified` forever: they
// are regenerated by `uv run` from the DEPLOYED dir, so pruning them is a one-shot
// and they come straight back. The ignore-list, not `--prune`, is what makes
// doctor stay green.
#[test]
fn all_seven_measured_residue_paths_are_ignored() {
    let measured = [
        "scripts/__pycache__/okf.cpython-314.pyc",
        "scripts/.pytest_cache/.gitignore",
        "scripts/.pytest_cache/CACHEDIR.TAG",
        "scripts/.pytest_cache/README.md",
        "scripts/.pytest_cache/v/cache/nodeids",
        "test-harness/.scratch/sandbox.env",
        "test-harness/topology.txt",
    ];
    for rel in measured {
        assert!(
            crate::marker::is_ignored_relpath(rel),
            "measured residue {rel} must be excluded from the tree hash"
        );
    }

    // Negative control: real skill content is NOT swallowed. The matcher keys on
    // path SEGMENTS, so a legitimate file whose name merely contains a token stays.
    for keep in [
        "SKILL.md",
        "scripts/okf.py",
        "test-harness/run.sh",
        "spec/topology.txt.md",
        "docs/scratch-notes.md",
        "scripts/pytest_cache_helper.py",
    ] {
        assert!(
            !crate::marker::is_ignored_relpath(keep),
            "{keep} is real content and must NOT be ignored"
        );
    }
}

// REQ-YF-MARK-005: `embed.rs`'s `#[exclude]` list is the FOURTH surface and cannot
// consume the shared constant (proc-macro literals), so this asserts the lockstep
// the comment there promises. Without the embed half, a release built where
// `bootstrap.sh` has run bakes a developer's `.scratch/sandbox.env` into the binary
// and ships it — fixing only the read-side surfaces hides that rather than fixing it.
#[test]
fn embed_exclude_list_stays_in_lockstep_with_the_ignore_list() {
    let embed_src = include_str!("embed.rs");
    for pat in crate::marker::IGNORED_PATH_PATTERNS {
        assert!(
            embed_src.contains(pat),
            "marker::IGNORED_PATH_PATTERNS has {pat:?} but embed.rs excludes no such pattern \
             — the embed would BAKE it into the released binary"
        );
    }
    // And nothing embedded is residue, on the real tree.
    for rel in crate::embed::all_relpaths() {
        let skill_rel = rel.split_once('/').map(|(_, r)| r).unwrap_or(&rel);
        assert!(
            !crate::marker::is_ignored_relpath(skill_rel),
            "embedded file {rel} is residue and must not ship in the binary"
        );
    }
}
