...
| **PLAN** | Scope plus findings are synthesized into a plan document — epics, dependency-wired issues, capability gates, upstream linkage. Two review passes run in order: a mechanical **conformance** check (`PASS` / `INCOMPLETE`), then an adversarial **red-team** (`APPROVE` / `REVISE` / `INVESTIGATE-MORE`). Both agents are read-only; only the main session writes files. |
