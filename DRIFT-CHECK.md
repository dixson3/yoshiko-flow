...
| `e-web-skill-counts` | `value-equal` | the skill-count claims in `web/content/pages/architecture.md` equal the real tallies from `skills/*/SKILL.md`, counted by the **site presentation category** the plugin derives (`web/plugins/skill_pages.py` `_site_category`: a `skill-group: beads` skill is `workflows` unless its name starts `yf-beads-` — then `beads`; `utility` / `markdown` pass through). The total "**N skills**" equals the number of `skills/*/` dirs carrying a `SKILL.md`, and each per-category count (`workflows (N)` / `beads (N)` / `utility (N)` / `markdown (N)`) equals the skills in that derived category. The site's per-skill pages and grouped `/skills/` index are auto-generated from the same frontmatter and never drift — only these hand-authored prose counts do. `frontmatter-contract` is the source of truth: a mismatch is the **web page** drifting (FAIL on `web-skill-facts`), never the skills. |
| `e-web-skill-groups` | `field-set-subset` | every site category the plugin can derive from `skills/*/SKILL.md` (via `_site_category`) — currently `workflows`, `beads`, `utility`, `markdown`, plus any new `skill-group` value that would pass through — is represented in the `web/plugins/skill_pages.py` group registry (`GROUP_ORDER`, `GROUP_LABELS`, and `GROUP_BLURBS`), so every group renders on the generated `/skills/` index and left-nav with a defined order + label + blurb rather than being silently appended unlabeled. A derivable category missing from that registry is the **plugin** drifting (FAIL on `web-skill-pages-plugin`), never the skills. |
...
|:------------------|:----------|
| `skills/*/SKILL.md` | `e-spec-compliance`, `e-skill-script-cli`, `e-formula-name`, `e-agent-ref`, `e-template-ref`, `e-json-contract`, `e-status-values`, `e-formula-vars`, `e-install-url`, `e-readme-layout`, `e-readme-prereqs`, `e-readme-usage`, `e-readme-desc`, `e-frontmatter`, `e-skillspec-skillmd`, `e-protocol-rule`, `e-web-skill-counts`, `e-web-skill-groups` |
