...
- [assets/residual-issues/](assets/residual-issues/) - Issue 6.1's eight residual findings this plan found and did NOT fix, plus `r9`/`r10` — the two the LANDING found, recorded as RE-004/RE-005 — each drafted as a full issue body, with a README mapping the original eight to their local beads. All ten are now filed upstream (#348-#353) and every bead carries an `external_ref`; the eight were grouped by shared cause into four issues.
...
- [records/landing-apply-outcome.json](records/landing-apply-outcome.json) - The three `land --apply` attempts as executed, per-step: attempt 1 reaching `L_RECONCILED` and halting at `verify-reconcile` (the drafts named the plan `plan-063`, not the full id); attempt 2 halting at `halt_class 5` on a digest the landing's own L4/L6 had moved; attempt 3 reaching `L_DONE` with every step measured. Kept because RE-004 and RE-005 assert states this file is the evidence for.
