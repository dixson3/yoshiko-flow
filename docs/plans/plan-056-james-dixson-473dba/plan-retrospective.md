...
| `frontloadable` | partial |
| `detected_by` | mechanical-check |
| `evidence` | `recheck-criteria --json` verdict envelope: `verdict: HARNESS_INCOMPLETE`, `class_a: 19`, `evaluated: 18`, `unjudged: [SC11c]`, per-row `detail: 'timed out after 300s'`. Reproduce with `uv run skills/yf-plan/scripts/plan_manager.py recheck-criteria docs/plans/plan-056-james-dixson-473dba --json` against a COLD FULL-tier cache. |
| `escape_class` |  |
| `adjudication` |  |
| `origin` |  |
| `culpability` |  |
