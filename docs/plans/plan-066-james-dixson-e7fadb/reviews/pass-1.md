...
16 concerns (5 high, 2 medium-high, 6 medium, 3 low).
...
| :-- | :-- | :-- | :-- |
| C1 | high | **0 of 17 criteria are executable clauses** — `doc_lint` returns 17 `verification-clause` warnings. plan-065 shipped `plan065_checks.py` with one verb per criterion. A plan whose subject is "instruments that report success without doing the thing" regressed the corpus norm. | Author `scripts/checks/plan066_checks.py` with one verb per SC; rewrite Verification as `<cmd> → exit 0`, or `manual: <why>`. |
| C2 | high | **SC13/SC14 are vacuously true today**, with zero work done — equally satisfied by never authoring the pages. Worse, SC13 omits `-r`: under system grep it exits **2** (`Is a directory`) having searched zero bytes, and a reader scores PASS. It only exits 1 on this machine because `grep` is aliased to ugrep. | Make both conjunctive and positive: assert the file exists AND makes the correct positive claim, then keep the negative grep with `-r` and an asserted exit 1. |
| C3 | high | **25 of 44 issues are named by no success criterion**, including **all six of Epic 3** (the entire manifest-repair half of Class B) and **all three of Epic 7**. | Add criteria for Epic 3 (a parser asserting the new node/edges/§6 rows exist and every §6 row names an existing edge) and for Epic 7. |
| C4 | high | **The plan repeats #317's undercount one level up.** The checker corpus is `web/content/**`, so `README.md:105-106` — carrying the identical drifted harness matrix — is structurally unreachable, and absent from 4.4's site list. `AGENTS.md:78` names the same retired roots. SC7 would certify "all 15 sites repaired" while the repo's most-read document stays wrong. | Make the checker corpus a parameter; run over `README.md` + `AGENTS.md` + `web/content/**`. Add `README.md`'s harness table to the `harness_desc.rs`-sourced node. |
| C5 | high | **Issue 6.4 would author a page that never renders, silently.** Measured: created `web/content/concepts/glossary.md`, ran the gate's exact Test → exit **0**, **32 pages** (unchanged), zero warnings, nothing emitted. `pelicanconf.py` sets no `PAGE_PATHS`, so the default `["pages"]` excludes any new directory. **The plan's own thesis firing inside the plan.** | Author under `web/content/pages/`, or add the directory to `PAGE_PATHS` in the same issue. Add a criterion asserting the emitted `index.html` exists and is non-trivial. |
| C6 | medium-high | **Issue 5.5 / SC12's `render.py check-dir` is vacuous.** Executed over the diagrams the plan calls wrong: `{"status":"ok","orphans":[],"stale_advisory":[]}`, exit 0. `render.py:154-169` exits non-zero **only on orphans**; staleness never affects exit code. And the human-read gate `Blocks: 5.5` — spending a human gate to unblock a check that passes today with no work. | Drop `5.5` from that gate's Blocks (keep `8.1`). Restate 5.5 as orphan-detection only. |
| C7 | medium-high | **SC3/SC10 depend on commit topology a squash destroys.** plan-064 landed as a single commit; 063/065 as `--no-ff` merges. Commit messages here are epic-level, so "Issues 2.3 and 3.4" cannot be located in a log. Meanwhile `check_amendment_log.py --plan` is a CV row for plans 060, 062, 063, 064 — 066 adds none. | Add `gate-plan066-amendment` + §3 globs and make SC3 `check_amendment_log.py --plan …` → exit 0. Stop reading commit topology. |
| C8 | medium | **Issue 0.1's "living-amendment-log entry" has no home.** Neither `spec/checks.md` nor `yf-drift-check/SPEC.md` has an amendment log; only root `SPEC.md` does, and it contains zero `REQ-CHECK-*` ids. | Name the amendment-log target file explicitly in 0.1 and reconcile it with `check_amendment_log.py`'s root-`SPEC.md` assumption. |
| C9 | medium | **The "Checkers are fail-capable" gate is weaker than its own Condition** and has no vacuity floor. Its Test asserts exit 0; SC4 demands "one observed failure per checker" — that half is not in the gate. A harness that silently skips a checker exits 0 and opens it. | `--all` must print and assert a per-checker observed-failure count and carry `--min-checkers N`; put it in the gate Test, not only in SC4. |
| C10 | medium | **The "Build green" gate Test is unsatisfiable in an execute worktree.** It hardcodes `web/.venv/bin/pelican`; `web/.venv` is untracked and exists only in the primary checkout. The gate blocking Epics 4/5/6 fails with exit 127, and no issue creates the venv. | Add the venv bootstrap to Issue 1.2, or switch to `uv run --with-requirements web/requirements.txt pelican …`. |
| C11 | medium | **Issue 2.6 self-inflicts a red validation window.** It lands recipe rows for checkers that stay red until 4.9/5.3. `_validate_merged` tier 1 returns `fail` on any non-pass, blocking any intermediate land from 2.6 onward. | Sequence 2.6 after 4.9/5.3, or state the expected-red window and forbid an intermediate land. |
| C12 | medium | **D6's bet is real but narrower than stated.** CV's FULL tier IS mechanically enforced by `_validate_merged`. But CI runs no tier (`grep change_validation .github/workflows/*` → nothing), and the FAST on-edit trigger is an always-loaded *prose* rule — the same class that produced 4-opportunities/0-catches. The coverage binds on the yf-plan land path only. | State the boundary in D6 and R5. Add a CI job running the four new `check_web_*` rows — the only coverage for non-plan commits. |
| C13 | medium | **A declared `not_checked` class with no discharge mechanism.** `architecture.md:65`'s `utility (7) — skill authoring, drift checking, …` is a prose membership list with the same semantic-mis-assignment risk the plan declares unchecked for diagrams — but the human-read gate is scoped to PNGs only. | Extend the human-read gate (or add a criterion) to `architecture.md`'s group lists, or have `check_web_counts.py` parse the enumerated names. |
| C14 | low-medium | #317 is `include`, but the only `resolves-upstream: #317` is on 1.2 marked `(partial)`. All five `include` rows carry `Resolved By: _TBD_` (R2a promotes to `E` at `reconciling`). | Add a full `resolves-upstream: #317 (include)` on 8.5 or 4.9, and fill the Resolved By column before close. |
| C15 | low | SC2 names "emitted `index.html`" — there are 33. No false pass at site root, so ambiguity not defect. | Name `output/skills/yf-okf-hygiene/index.html` explicitly. |
| C16 | low | SC12 asserts what the plan's prose does **not** say — unfalsifiable by execution, vacuously true from the moment written. | Delete it; the prohibition already lives in Issue 5.5's text. Or restate as a grep over the plan document with an asserted exit 1. |
