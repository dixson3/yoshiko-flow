...
- [assets/](assets/) - The plan's own executable harness and its records. Not diagrams (those live in `diagrams/`), not attachments — these are run by the plan's gates.
  ...
  - [assets/sc15-full-validation.md](assets/sc15-full-validation.md) - SC15: the FULL tier over the merged tree, 45/45.
  - [assets/sc20-deploy.md](assets/sc20-deploy.md) - SC20: the deploy, the version stamp (MATCH, exemption not invoked), the consent-gate outcome (did not fire), and the structural verification that all three fixes are live and byte-identical.
  - [assets/closable-sweep.md](assets/closable-sweep.md) - The land-the-plane `closable` sweep. Proposals only — NOTHING was closed, and three of its seven proposals would have been wrong, including `#150`, which `verify-reconcile` requires to stay OPEN in the same session.
