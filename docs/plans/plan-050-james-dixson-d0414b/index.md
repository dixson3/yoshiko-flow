...
- [upstream-triage.md](upstream-triage.md) - The candidate pool from the automated scan (49 sections). The rows **plan.md's Upstream Issues table acts on** carry a disposition and reasoning mirrored from that table, which is authoritative; every other section is an unscored candidate and is deliberately blank. No count is stated here — it went stale twice (pass-6 C58).
