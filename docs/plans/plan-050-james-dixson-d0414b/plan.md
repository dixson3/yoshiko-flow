...
- Issue 2.1: Author the fixture `ctl-181-silent-green` — a bundle copied outside `docs/plans/` — and run it against the **unfixed** `doc_lint`, recording the byte-identical PASS that EXP-003 measured. **Run `redcheck.sh record-red <fixture> ctl-181-silent-green` against the unfixed tree; this issue PRODUCES that record** (pass-7 C68).
  - depends-on: 0.2
- Issue 2.2: Add the distinguishing verdicts to `doc_lint.py` behind an **opt-in `--require-selection` flag** — unflagged callers keep today's behaviour exactly; a caller that passes the flag gets `NOT_SELECTED` for a real-but-unselected path or root, `NO_SUCH_PATH` for a nonexistent one, and a non-zero exit. **This is #181's own option 2, and pass-9 C86 built it and measured it green.** Two earlier scopes were measured and BOTH fail: the general `files_checked == 0` form breaks `_shared/test_doc_lint.py`'s **SC42** (pass-8 C77), and a **`--path`-keyed-always** form breaks the same file's **SC17** block (`:722-743`), which pins an unselected `--path` to `PASS`/rc 0 and *identical to a nonexistent path* — the exact opposite of this plan's SC6. `doclint-tests` is in **both** the FAST and FULL tiers, so either scope fails the on-edit gate for every `doc_lint.py` edit and fails SC15; C86 ran the real FAST tier and got `doclint-tests FAIL rc=1`. The flag design needs **zero** edits to `test_doc_lint.py` (SC17 and SC42 both stay green, measured `all passed`), covers the **`--root`** form so #181's *titled* scenario — a bundle **copied** outside `docs/plans/` — is actually closed, and leaves `DOC-LINT.md`'s "ordinary, not exceptional" `files_checked: 0` case true for unflagged callers, including this plan's own Epic-6 `log.md` edits. It also confines the exit-2 widening to callers that explicitly asked for loudness, which sits far more comfortably with REQ-DATA-024's pinning of `INCONCLUSIVE` to "could not run". Selection semantics are **unchanged** — only their reportability, and only on request. **Then run `redcheck.sh assert-distinguishes <fixture> ctl-181-silent-green` against the fixed tree and record the zero observation** — this issue PRODUCES that record. **Named surfaces:** `_shared/doc_lint.py`, its byte-identical vendored copy `skills/yf-plan/scripts/doc_lint.py` (guarded by `_shared/sync.py --check`), and `skills/yf-plan/protocols/DOC-LINT.md`. **`_shared/test_doc_lint.py` requires NO change under this design** — verified by execution; that is the design's main argument over the two rejected scopes.
  - depends-on: 2.1
  - resolves-upstream: #181 (include)
- Issue 2.3: Re-measure the corpus `files_checked` figure after the verdict change and record whether it moved. Also run `uv run _shared/test_doc_lint.py` and the FAST tier over a `doc_lint.py` change, and record both — pass-9 C86 measured that the two rejected scopes fail them, so this is the regression that pins the flag design. A change would mean selection was perturbed, which 2.2 forbids.
  - depends-on: 2.2
...
| SC2 | Every control shipped by Epics 1-3 was observed RED on a fixture, with the RED recorded by an issue that is a `depends-on` ANCESTOR of the fix | `assets/red-prework.md` records a `record-red` non-zero exit per control, with the command; the "before the fix" ordering is carried by the DAG edges 1.1→1.2/1.3, 2.1→2.2, 3.1→3.2 and not by any timestamp (pass-8 C83) | 0.2, 1.1, 2.1, 3.1 |
| SC2b | Every control was then observed GREEN, and the two observations are distinct records | for each id in `assets/controls.txt`, `assets/red-prework.md` carries a `record-red` record with a **non-zero** exit and an `assert-distinguishes` record with a **zero** exit — `redcheck.sh verify-all` asserts exactly this and exits 0/1/2. The claim is the exit-code distinction, **not** a temporal ordering: pass-7 C69 measured the HEAD-hash ordering check as vacuous when the fix is uncommitted. Pass-5 flagged this half as having no criterion at all | 1.2, 1.3, 2.2, 3.2a, 1.4, 2.4, 3.4 |
...
| SC6 | Under `--require-selection`, a real-but-unselected path, a nonexistent path, and a selected one are three distinguishable outcomes — **including the `--root` form** | `doc_lint --path AGENTS.md --require-selection` → `NOT_SELECTED`; `--path docs/plans/NO-SUCH/plan.md --require-selection` → `NO_SUCH_PATH`; `--root <a bundle copied outside docs/plans/> --require-selection` → `NOT_SELECTED` (this arm is #181's **titled** scenario, which a `--path`-only scope leaves silent — pass-9 C86); and **without** the flag all three keep today's behaviour, which is what leaves `test_doc_lint.py` SC17/SC42 green | 2.2, 2.4 |
