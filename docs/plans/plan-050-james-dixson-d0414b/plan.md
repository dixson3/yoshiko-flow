...
- Issue 1.4: **Verify** — assert `assets/red-prework.md` **contains** both records for each of this epic's two controls (the RED from 1.1, the GREEN from 1.2/1.3), and assert `_bead_is_terminal` is unmodified. This issue reads the file; it runs no `redcheck.sh` verb, so no gate evidence is produced inside the gate's own Blocks set. Pass-5 C38: an earlier draft had it recording the GREEN observation, which put the gate's evidence inside the gate's own Blocks set — a REQ-AGENT-046 cycle.
  - depends-on: 1.2, 1.3
...
- Issue 2.1: Author the fixture `ctl-181-silent-green` — a bundle copied outside `docs/plans/` — and run it against the **unfixed** `doc_lint`, recording the byte-identical PASS that EXP-003 measured. **Run `redcheck.sh record-red <fixture> ctl-181-silent-green` against the unfixed tree; this issue PRODUCES that record** (pass-7 C68).
  - depends-on: 0.2
- Issue 2.2: Add the distinguishing verdicts to `doc_lint.py`, **keyed on `--path` only**. Scope is stated because it is load-bearing (pass-8 C77): the general `files_checked == 0` form breaks `_shared/test_doc_lint.py`'s **SC42** assertion, which pins `files_checked: 0, verdict: PASS, rc: 0` for a `--root` invocation with no `--path`, and `doclint-tests` sits in **both** the FAST and FULL tiers of `CHANGE-VALIDATION.md` — so the general reading fails the on-edit gate for every `doc_lint.py` edit and fails SC15. `--path` is also the form exp-003 measured and the form #181's reproduction uses. Reuse the existing 0/1/2 **exit-code** vocabulary rather than inventing a fourth exit code — this is about exit codes, not verdict strings, which are a separate closed set (see 0.1's REQ-DATA-024 amendment). Selection semantics are **unchanged** — only their reportability. **Then run `redcheck.sh assert-distinguishes <fixture> ctl-181-silent-green` against the fixed tree and record the zero observation** — this issue PRODUCES that record. **Named surfaces this issue must touch or explicitly leave alone:** `_shared/doc_lint.py`, its byte-identical vendored copy `skills/yf-plan/scripts/doc_lint.py` (guarded by `_shared/sync.py --check`), `_shared/test_doc_lint.py`'s SC42, and `skills/yf-plan/protocols/DOC-LINT.md`, whose §table asserts `files_checked 0 → PASS → exit 0` and calls that case "ordinary, not exceptional" for the reserved `index.md`/`log.md` in every bundle — an always-loaded on-edit protocol this change would otherwise falsify. **Then run `redcheck.sh assert-distinguishes <fixture> <control>` against the fixed tree and record the zero observation** — this issue PRODUCES that record (pass-6 C53); the gate's blocked issues only consume it.
  - depends-on: 2.1
