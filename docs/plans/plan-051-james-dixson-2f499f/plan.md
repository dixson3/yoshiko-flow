...
- Issue 1.1: Author the fixture `ctl-182-spike` with **two conjuncts** — (a) `red-team.md` authorizes a spike, and (b) **every double-quoted fragment in `REQ-AGENT-043`'s `Verification:` line resolves in the file it names**. **A fragment containing an editorial ellipsis is matched as a REGEX with `...` → `.*?`, not verbatim** — this is not a detail: measured at drafting, that line's middle fragment is `"writes ... at presentation"` and `grep -cF` for it returns **0**, so a literal-only conjunct is **red on every tree**, including a fully fixed one. Pass 1 built the fixture to the letter and got exit 1 on all three arms — a false RED, a half-fix arm carrying no information, and a capability gate that could never open (pass-1 C2). Issue 1.2's retarget must likewise leave every fragment resolvable under this rule. Conjunct (b) is the substance: EXP-002's spike measured the FAST tier returning `pass, first_failure None` on the dangling state where `red-team.md` was reworded and the spec still pinned the old string. **Run `redcheck.sh record-red <fixture> ctl-182-spike` against the unfixed tree; this issue PRODUCES that record.**
  - depends-on: 0.2
...
| SC7 | The Phase-3 wisp **represents the review loop**, its arms are **sequential**, and it carries no counter | `bd cook --dry-run` succeeds AND its emitted step-id set **equals** the named set {conformance, red-team, resolve, gate} in a chain with the gate terminal — a **set** assertion, per 3.3's own never-a-count rule. THEN the negatives: no step has more than one `needs` entry, and no step or var records a review-cycle count. The positive clause is required because two negatives are **vacuously true of an empty formula** — a zero-step file cooks, has no multi-`needs` step and records no counter (pass-1 C6) | 2.3 |
