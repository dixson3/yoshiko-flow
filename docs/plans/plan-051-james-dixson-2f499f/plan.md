...
| SC2b | Every control was then observed GREEN, and the two observations are distinct records | for each id in `assets/controls.txt`, both a non-zero `record-red` and a zero `assert-distinguishes` record exist — `redcheck.sh verify-all` asserts exactly this and exits 0/1/2 | 1.2a, 2.3, 3.3, 1.4, 2.4, 3.4 |
