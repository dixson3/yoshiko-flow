...
- **`skills/yf-change-validation/scripts/change_validation.py:946`** declares `--changed` with `nargs="*"` and **no** `action="append"`,
...
- Issue 2.3: Ship `skills/yf-plan/formulas/plan-review.formula.toml` — the Phase-3 wisp, giving the review loop the bead representation it is the only phase to lack. **Arms stay SEQUENTIAL (D-7):** conformance → red-team → resolve → gate, one reviewer per cycle, preserving the chain property in which each pass verifies the previous pass's resolutions. **The red-team step is TWO beads, `dispatch-pass-N` blocking `record-pass-N`** — so `record` never enters `bd ready` until `dispatch` closes, and `dispatch` cannot close until the agent returns. EXP-005 measured this exact behaviour (the join stayed absent from the ready set until every arm closed). The honest limit: it binds the TRACKED workflow, not prose — a bead cannot stop a session *claiming* it dispatched. What it buys is that "what is next" comes from `bd ready` rather than from an actor's memory of what it intended, which is where both observed misses came from. **The formula holds exactly ONE iteration; the coordinator owns the loop.** Measured on bd 1.1.2: there is no `loop`/`repeat`/`while`/`iterate` primitive, `until` is an unrelated cross-project capability dep that does not gate readiness, gate types are `human|timer|gh:run|gh:pr` with none re-running a shell command, and `bd dep add` runs cycle checks — **a cycle is unrepresentable by construction**. Both shipped formulas are skeletons whose steps are injected at runtime, so the loop is unrolled by re-pouring per cycle, not iterated inside the formula. Say this in the formula's own comments so a later reader does not try to build the cycle into it and rediscover that it cannot. `needs` is an array and multi-parent fan-in works (EXP-005 drove it), but this plan **does not use fan-out**. **The counter does NOT move into the wisp** — see Non-goals. Scripted `bd mol burn` **must pass `--force` and must check output, not the exit code**: measured, a cancelled burn on a wisp with an open APPROVE gate exits **0**.
  - depends-on: 2.2
...
| SC4b | **The hand-enumerated edit set is CLOSED** — every surviving `never writes files` restatement under `skills/yf-plan/**` and `web/content/**` is an enumerated row in `assets/edit-set-182.md` with a stated disposition | run the pattern in **§ SC4b's command** below from the repo root; assert the hit set is **NON-EMPTY** (the vacuity guard SC7, 3.2 and 3.3 all carry) **and** a **subset of the PATHS NAMED BY** that file's rows. A path-to-path comparison: the rows are `file:line` sites, projected to paths (pass-5 C43). A subset assertion, not a count (3.3's rule). **Stated blind spot:** it catches a new unenumerated FILE, never a new unenumerated LINE in an already-enumerated file. This extends SC4's coverage to the two paths the pattern actually reaches — `skills/yf-plan/SKILL.md` and `web/content/pages/workflows.md`. **It does NOT cover `skills/yf-plan/SPEC.md:65/389-390`, `glossary.md:90` or `web/content/skills/yf-plan.md:34`**, which carry *"Both agents are read-only"* and no form of *"never writes files"*, so they are not in the hit set. Those three stay hand-enumerated under R1 and are stated there rather than presented as closed (pass-5 C44 — an earlier draft claimed them) | 1.2, 1.2a |
...
| SC7 | The Phase-3 wisp **represents the review loop**, its arms are **sequential**, and it carries no counter | `bd cook --dry-run` succeeds AND its emitted step-id set **equals** the named set **{conformance, dispatch, record, resolve, gate}** in a chain with the gate terminal — **five**, because the red-team step is two beads (`dispatch` blocks `record`), which is the ordering constraint 2.3 ships — a **set** assertion, per 3.3's own never-a-count rule. THEN the negatives: no step has more than one `needs` entry, and no step or var records a review-cycle count. The positive clause is required because two negatives are **vacuously true of an empty formula** — a zero-step file cooks, has no multi-`needs` step and records no counter (pass-1 C6). **Stated limit (pass-2 C28):** four empty steps with the right ids in the right order would still satisfy this. Under **D-7** that is close to the whole deliverable — the wisp is scoped to *sequencing*, so step identity and order ARE the substance — but each non-gate step must additionally carry a non-empty description — **read from `plan-review.formula.toml` DIRECTLY, not from `bd cook --dry-run`**, whose output is only `step-id: Title (type)` and carries no description field, so the clause is not observable from the instrument SC7 names (pass-5 C45) | 2.3 |
...
Stated as a fenced snippet rather than inside a table cell **because that is what broke it** (pass-4
C36): the first version lived in a `|`-delimited row, so its alternation was written `\|` to escape
the table separator — and in an `-E` regex `\|` is a **literal pipe**. Measured, it matched nothing
and returned exit 1, making SC4b **unfalsifiable**: an empty hit set is a subset of every row set.
The M5 "criterion that cannot fail" class, inside this plan's own criteria.

```bash
# from the REPO ROOT
git grep -lniE 'never writes? (any )?files' -- 'skills/yf-plan/**' 'web/content/**'
```

**Measured at pass 4: exactly 7 paths** — `skills/yf-plan/SKILL.md`, `agents/captor.md`,
`agents/red-team.md`, `agents/reviewer.md`, `spec/agents.md`, `spec/portability.md`,
`web/content/pages/workflows.md`. Five are already `edit-set-182.md` rows; **`agents/captor.md` and
`spec/portability.md` are added as explicit NO-EDIT rows** — they carry the *captor's* read-only rule,
a different agent, out of scope for #182.

**Stated tradeoff.** Dropping the `read-only` alternant narrows coverage to the "never writes files"
phrase family. The broader pattern returns **30** paths against 8 enumerated rows — 22 of them
`scripts/fixtures/classify/*/plan.md`, `plan_manager.py`, test files, and `web/content/**` pages about
*other* skills. Restoring `read-only` would need a `':!skills/yf-plan/scripts/**'` exclusion plus nine
further no-edit rows; that is a worse trade than the narrower phrase, and it is recorded rather than
silently taken.
