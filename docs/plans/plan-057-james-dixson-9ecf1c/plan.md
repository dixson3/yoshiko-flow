...
| SC0c | The three `auto` gates were actually POURED `test_class: probe` — the directive is prose to the coordinator, and until now nothing asserted the poured beads honoured it. A directive nobody checks is the same class of defect as the missing directive pass 1 found. | `test -x scripts/checks/check-gates-poured-probe.sh && bash scripts/checks/check-gates-poured-probe.sh docs/plans/plan-057-james-dixson-9ecf1c` → exit 0 | 1.0 |
