...
type: Reference
okf_spec: OKF-PLAN
description: 'Upstream issue #325 - `gate_consistency.py` returns PASS on gates it
  cannot evaluate — a vacuous check'
---
# Upstream #325: `gate_consistency.py` returns PASS on gates it cannot evaluate — a vacuous check
...
Related: **#305** — a different defect in the same script (`--json` not honoured on the INCONCLUSIVE path). Both concern `gate_consistency.py` reporting less than it appears to.
