...
type: Reference
okf_spec: OKF-PLAN
description: 'Upstream issue #326 - `land`''s `draft_body_path` posts bundle files
  verbatim, but OKF requires them to carry frontmatter'
---
# Upstream #326: `land`'s `draft_body_path` posts bundle files verbatim, but OKF requires them to carry frontmatter
