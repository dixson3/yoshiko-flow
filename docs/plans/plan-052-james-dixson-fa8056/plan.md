...
| #221 | yf-plan: SC24-style criteria assert a MOVING fact where they should assert a DURABLE one | deferred | Deferred defect D10, found by the completion commit itself. SC24 is left AS-IS and the plan stays `complete` — the operator ruled its falseness is recorded upstream rather than papered over. Filed under grant AMENDMENT 4 | 7.3 |
...
| SC1c | The commit touching `skills/*/spec/**` or `skills/*/SPEC.md` PRECEDES the first commit touching any OTHER `skills/**` path — checked **PRE-MERGE and PRE-SQUASH at 7.1**. The earlier form ("before the first `skills/**` commit") was false by construction: 0.1's own touches are all under `skills/**`. **Checked on a range that outlives the merge:** `<base>..HEAD` is empty once the branch lands and HEAD is the base, so the control falls back to the merge's parent range `M^1..M^2` — the same commits, same order, permanently resolvable, no literal sha. **Under a SQUASH merge it returns INCONCLUSIVE (exit 2), never a pass and never a fail**, because the branch's commits do not survive as a second parent and commit ORDER — the whole claim — is unrecoverable. §6.1 mandates `--no-ff`, so that arm does not fire today | `bash docs/plans/plan-052-james-dixson-fa8056/assets/gate-run.sh run ctl-spec-first-order` → exit 0 | 0.3, 7.1 |
...
| SC20 | Every upstream row reached the end state its disposition requires | `uv run skills/yf-plan/scripts/plan_manager.py verify-reconcile docs/plans/plan-052-james-dixson-fa8056 --json \| jq -e '([.rows[]\|select(.verdict=="fail")]\|length)==0 and (.rows\|length)>0'` → exit 0 | 7.3 |
