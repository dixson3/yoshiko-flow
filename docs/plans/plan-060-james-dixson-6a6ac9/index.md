...
- [assets/free-req-ids.md](assets/free-req-ids.md) - Repo-wide `REQ-*` allocation table (45 families across skills/, SPEC.md, yf/src, scripts/, _shared/) computed in Python rather than grep, plus the five ids plan-060 allocates and why REQ-PLAN-083 skips 082. A dated allocation-time snapshot, deliberately not rewritten.
- [assets/cited-figures.md](assets/cited-figures.md) - Machine-readable registry of every re-measurable figure this bundle quotes, paired with the command that re-derives it. Read by scripts/checks/check-cited-figures.py (#289); its first run caught req-families drifting 45 -> 46.
- [assets/rehearsal-record.json](assets/rehearsal-record.json) - Machine-readable record of the Epic-6 rehearsal: the fake-origin URL, the terminal journal state (L_DONE), every step executed, and which steps were stubbed or skipped. Read by SC36 and SC36b — without a commissioned artifact those tests would assert something they invented.
- [assets/full-tier-record.md](assets/full-tier-record.md) - Dated wall-clock measurement of the CHANGE-VALIDATION FULL tier — a figure D-8 found recorded nowhere in the repository — with the truncation caveat that the runner breaks on first failure, so the number is a lower bound.
- [assets/upstream-grant.md](assets/upstream-grant.md) - Operator authorization for plan-060's own upstream reconcile writes (2026-08-30): nine comments, and NO issue closures. Records the withheld #301 close as a DECISION rather than an omission, and states that the gate's own Test passes unreliably here.
- [assets/landing-runbook.md](assets/landing-runbook.md) - Operator runbook for land: what the session prints, what you paste, every exit code, and a resume row for all seventeen journal states including the four non-uniform conflict recoveries.
