...
| C5 — four figures false, one fixture gone | low | **Accepted — and this cell was itself INACCURATE, caught by pass 7.** It originally claimed *"the 37s are marked \"AT THE TIME. Now 0\""*; `grep -c "AT THE TIME" plan.md` returned **0**. The substance landed in different words (the retirement sentence), so it was a recording defect, not a missing fix — but it is the **second consecutive** resolution cell describing an edit that is not present, and it sat immediately below a cell asserting *"every edit in this round is assert-guarded"*. **An assert on the edit string cannot catch a resolution cell that describes a different edit.** The 37s are now gone entirely: Issue 1.9 was rewritten around the spike rather than appended to, which removed the worked example that carried them. the "cheapest fixture sitting in the repository" claim is replaced with a statement that the commit destroyed it and SC10b must construct its own. Recorded as R12 / Issue 0.10's class caught live — a derived figure in prose, invalidated by an event three hours later. | `main-session` | `resolved` |
