...
| C4 — superseded prescription and a drifted count | low | **Accepted — and the forward-pointer half of this resolution DID NOT LAND.** *(Corrected after red-team pass 6 caught it.)* The edit silently no-opped on an unmatched string and was recorded here as done: **#250's class — a resolution recorded as done and never written.** It is now applied and verified present, and the pointer itself carries a note saying it was missed once. The count half did land; the bundle count is given as "36 when first measured, 37 by the next review pass" with a note that the figure drifts as review files are added and is therefore not the claim. | `main-session` | `resolved` |
