...
- Issue 1.7: Emit the **fully-qualified `--apply` command** the operator must run, including the checkout it must be run from. The session prints this and stops; an ambiguous cwd is the difference between merging in the primary checkout and attempting it in a worktree that cannot check out the target branch.
  - depends-on: 1.6
- Issue 1.9: **Enumerate deliberately — the question, the checkout AND the flags all matter.** Settled empirically in [assets/enumeration-spike.md](assets/enumeration-spike.md) (six fixture cases, every candidate, both cwds, against known answers) after the prescription was wrong in five consecutive prose-reasoned rounds. **(a) Which question.** *Tracked-ness* (`git ls-files`) and *presence* are different facts; `--others` is the exact **complement** of `ls-files`, so **neither is ever a presence fact alone** — each returned 2 of 4. **(b) Which checkout.** A linked worktree carries a `.git` marker, so the primary checkout's git reports it as one opaque entry and never descends: measured **0** from the primary cwd for every git candidate — and **still 0 with the worktree NOT gitignored at all**, which is the control that identifies the cause. The boundary git cannot cross is the **CHECKOUT** boundary, not the gitignore boundary; git crosses a plain gitignored directory correctly (`--others --ignored --exclude-standard` -> 2 of 2). **(c) The prescription.** Prefer **`git -C <that worktree> ls-files --cached --others --exclude-standard`** — a correct, atomic union that handles symlinks as git does and excludes ignored junk. Only where the process cannot run inside that checkout, fall back to an explicit scoped listing written **`find <dir> ! -type d`** — never `-type f`, which **drops symlinks**, and accept that it counts ignored junk (`.DS_Store` is near-certain on macOS). **A reader who checks this against the live repository will find the two `find` forms AGREE** — both return 43, because this bundle happens to contain zero symlinks and zero `.DS_Store` files — **and must not conclude the distinction is pedantic.** The difference is visible only in the fixture, which is exactly why the fixture exists: the live tree can only ever be in one state, and it is currently in the state where the wrong tool looks right. That is the same trap that let the enumeration prescription be wrong five consecutive rounds. The two branches answer different questions and the choice is deliberate: the listing answers *what is on disk*, the `git -C` form answers *what git considers part of the tree*. For `upstream.rows[].draft_present` the latter is wanted — a `.DS_Store` is not a draft. **(d) Never** `ls-files` alone, never `--others` alone, never any git form from a **different checkout**, never a recursive content grep (measured: the harness `grep` wrapper under-reports across gitignore, and `/usr/bin/grep -r` across both roots **double-counts** — 6 logical paths twice), and never `status --porcelain=v2 --ignored` as a fallback: it is the one git command returning non-zero from the primary cwd (**1**), and that 1 is the ignored **directory**, not its files — and `--ignored=matching`, the documented fix for exactly that collapsing, **also returns 1**. **(e) The premise this issue originally rested on is RETIRED**: it claimed draft bodies are *"untracked by construction at `--dry-run` time"*, which `commit-plan` falsifies by design and the operator falsified in practice mid-review. **An omission from enumeration is silent — it is not a `skip`, so the "every skip is surfaced in the consent prompt" guarantee does not cover it.**
  - depends-on: 1.1
  - resolves-upstream: #263 (partial)
...
| SC10b | The manifest enumerates via git plumbing: no `grep -r`/`grep -R` in the enumeration path, and a fixture carrying **a tracked draft, an untracked draft, a SYMLINKED draft and an ignored `.DS_Store`** inside a linked worktree is enumerated **exactly** — all three drafts, not the junk — **with the enumerating process's cwd pinned to the PRIMARY checkout** — every element is load-bearing and each defeats a specific wrong implementation: the cwd pin defeats a `git -C <worktree>` shortcut, the **tracked** draft defeats `--others`-only, the **untracked** draft defeats `ls-files`-only, the **symlink** defeats `find -type f`, and the **`.DS_Store`** defeats `find ! -type d` used without filtering — without that pin a test author can enumerate with `git -C <worktree>` and make the word "gitignored" vacuous, which is how this blindness survived three rounds. | `bash scripts/checks/check-pytest-ran.sh skills/yf-plan/scripts/test_land_manifest.py test_enumeration_uses_git_plumbing` -> exit 0 | 1.9, 1.8 |
