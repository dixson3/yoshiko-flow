...
fingerprint: e05f45348b1aa974f3708417ea4e6820eac5e2ad23f103e10736bcb207b33da1
epic: yf-mol-vltm
---
...
| SC5 | Escalations **batch**: two or more raised, and **fewer pushes than escalations**. **AMENDED AT RECONCILE (operator-authorized), and the history is kept because it is the point.** The shipped form asserted `.pushes <= 1` **CUMULATIVELY** while batching is a **PER-BOUNDARY** property — so the criterion could only ever hold if the plan escalated at exactly **one boundary in its entire lifetime**. Measured at the moment of amendment: `raised 6, pushes 3` across three distinct `push_batch` ids — **2:1 batching working correctly**, which the criterion read as failure. This cell previously recorded that `>= 1 and < .raised` was rejected by an earlier pass because *"Issue 2.5 raised exactly one escalation so `pushes < 1` could never hold"*. **That rejection was CORRECT AT n=1 AND WRONG AT n=6** — the plan's own execution falsified the assumption the rejection rested on, which is exactly what a plan that escalates about its own escalations should expect to happen. `.pushes < .raised` is the property the criterion always meant: fewer pushes than escalations proves batching **at any n**. It is topology-independent in the way the earlier forms were reaching for — satisfied by a real batched push, unsatisfiable by a per-escalation push, and the `YF_PARENT_PANE`-unset arm raises no escalations at all, so the antecedent never arises. | `set -o pipefail; uv run skills/yf-plan/scripts/plan_manager.py escalation-report docs/plans/plan-059-james-dixson-55137e --json \| jq -e '.raised >= 2 and .pushes < .raised'` -> exit 0 | 2.5, 3.3, 3.5 |
