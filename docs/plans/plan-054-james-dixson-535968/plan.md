...
| SC6 | **No instruction surface a reader may copy from** invokes a script by a hardcoded relative `.claude/skills/` path. The set is **DERIVED** — every `SKILL.md`, every skill `README.md`, **and the project `README.md`** — because the original wording enumerated only the first two and a real functional bug for pi/opencode users (3 sites in the project README) therefore sat *outside the criterion written to prevent exactly it* | `bash docs/plans/plan-054-james-dixson-535968/assets/checks/check-no-hardcoded-skillpath.sh` → exit 0 | 1.5 |
...
| SC29 | **AMENDED at close (operator decision).** Originally: *"The `v0.5.0` tag exists and points at the validated merge commit"*, discharged by 6.8. **6.8 is DESCOPED** — the operator defers the tag to a successor session so the harnesses can be verified manually under a real HOME — so the original wording is now FALSE, and a criterion that is false because its work was deliberately deferred is a criterion that no longer states what the plan means. `SKILL.md` §6.4 sanctions exactly this: *amend the criterion if it no longer states what the plan means*. It now reads: **the release is STAGED and validated at one commit, and the tag is DEFERRED rather than pushed** — versions agree on 0.5.0 across crate, `Cargo.lock`, `CHANGELOG.md` and the site constant; the branch is fully pushed; and **no `v0.5.0` tag exists**, because pushing one is irreversible and auto-publishes the website. The deferred work is tracked upstream and cross-linked from #236 | `bash docs/plans/plan-054-james-dixson-535968/assets/checks/check-release-staged.sh` → exit 0 | 6.5, 6.5a, 6.6 |
