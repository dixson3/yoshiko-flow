...
## Objective
Replace the fully auto-generated per-skill web pages — today synthesized at build time by
`web/plugins/skill_pages.py` from each skill's `SKILL.md` frontmatter + `README.md` body — with
**explicit, hand-editable pages** under `web/content/skills/<name>.md`, one per skill, seeded
richly from `skills/<name>/{SKILL.md,README.md,SPEC.md}`. A repo-root `DRIFT-CHECK.md` edge keeps
those pages honest as each skill's behavior/implementation evolves.

Adopt a **hybrid composition** (operator decision): the plugin keeps auto-injecting the
frontmatter-derived **"At a glance"** block (group, invocation, requires-tools, depends-on,
dependents, source links) — mechanical facts that literally cannot drift — and keeps generating
the **`/skills/` index** and the **`SKILL_NAV`** sidebar from frontmatter (count/nav can never
drift). Everything else on a skill page becomes **authored prose** in `web/content/skills/<name>.md`,
and the drift-check edge covers that authored prose only.

## Motivation
The skills pages are the only pages on the site with **no hand-editorial control**: the plugin
transforms `SKILL.md` + `README.md` into a page at build time, so the operator cannot make a
skill-specific edit (a clarifying example, a reordered section, a cut) without editing the skill's
own `README.md` — which is developer-facing and shared with the installed skill, a different
audience and register from the public site. The result is that skill pages read like transformed
reference docs, not authored site pages, and cannot be tuned per skill.

The operator wants real, per-skill authored pages they can edit directly. The risk of authored
pages is the inverse of the current design's strength: authored prose can **drift** from the
skill's actual behavior as the skill evolves. The repo already runs `yf-drift-check` (an
`approved: yes` root `DRIFT-CHECK.md`), so the fix is to add a source-of-truth edge from each
skill's `{SKILL.md,README.md,SPEC.md}` to its authored page — the same mechanism that already
guards `README ↔ SPEC ↔ implementation` across the repo. Hybrid composition keeps the mechanical
catalog facts auto-fresh (no drift possible) so the drift-check burden is scoped to prose only.
...
## Scope decisions (operator, 2026-07-24)
- **Composition model: HYBRID.** Plugin auto-injects the frontmatter-derived "At a glance" block
  and keeps generating the `/skills/` index + `SKILL_NAV`; the authored `web/content/skills/<name>.md`
  owns all prose. Drift-check covers the authored prose only (mechanical facts can't drift because
  they stay generated).
- **Seed depth: RICH.** Each page is seeded as a substantial draft to trim, not a stub to grow:
  summary + when-it-fires / when-to-skip (from `description`) + the README exposition (why / how /
  usage) + salient `SPEC.md` behavior highlights.
- **Drift edges: globbed + per-skill pairing, one edge per source node (schema-clean).** The
  operator's intent — concise, per-skill-paired, **not** 18 explicit edges — is honored, but the
  manifest schema requires every §2 edge to reference **§1 node IDs** (`Source Node → Derived
  Node`), so a single edge sourced from the raw glob `skills/*/{SKILL.md,README.md,SPEC.md}` is
  malformed (that triplet is three existing nodes, not one). The schema-clean form is **three**
  glob edges — `e-skill-page-desc`/`-readme`/`-spec` — each from an existing node
  (`skill-md`/`skill-readme`/`per-skill-spec`) → a new `skill-page` node, name-paired by `*`. Three
  concise rows, still per-skill-paired, not 18.
- **Boundary:** web content + the `skill_pages.py` plugin + the `DRIFT-CHECK.md` manifest. No
  change to any skill's `SKILL.md`/`README.md`/`SPEC.md` (those are the sources of truth being
  paged, not edited here). VOICE.md (plan-035) governs the authored prose.

## Investigation Findings
No sub-agent experiments were needed — the mechanism is fully understood from reading the source,
and the open questions were design choices resolved in scoping. Grounding facts:

- **Current generator (`web/plugins/skill_pages.py`).** Reads every `skills/*/SKILL.md` at build
  time; emits (a) one page per skill at `/skills/<name>/` and (b) a grouped `/skills/` index, and
  builds `SKILL_NAV` for the sidebar. Page body today = optional `content/skills/<name>.md` intro
  layered **above** + frontmatter-derived summary + **"At a glance"** block (group, invocation,
  requires-tools, `depends-on-skill`, reverse `dependents`, `SKILL.md`/`README.md` source links) +
  when-it-fires / when-to-skip (split from `description`) + the **README body** auto-transformed
  (dev-only `Install` / `File Layout` sections dropped, skill-relative links rewritten to GitHub).
  Registered on `page_generator_finalized`. `_skill_override_html` already reads
  `content/skills/<name>.md` — the seam we invert.
- **Hooks are per-signal, well-isolated.** `PLUGINS = [sitemap, home_content, skill_pages]`;
  `STATIC_PATHS=["images","extra"]`; pages live in `content/pages/`. `content/skills/` exists with
  a `.gitkeep`.
- **Drift-check is already live.** Root `DRIFT-CHECK.md` is `approved: yes` (the reference/
  regression instance for the engine), with §1 Artifact Nodes, §2 Source-of-Truth Edges, §3
  Per-Edge Contracts, §5 Required-Section Contracts, §6 Trigger Scope (changed-path globs), §7
  Fixed-Authority Policy. Node/edge style is glob-based (e.g. `skill-md = skills/*/SKILL.md`,
  `skill-readme = skills/*/README.md`, `per-skill-spec = skills/*/SPEC.md`), and **every §2 edge
  references §1 node IDs** (`Source Node → Derived Node`) with a `Contract`/`Check Category` from a
  fixed vocabulary. We ADD one `skill-page` node + **three** node-sourced edges
  (`e-skill-page-desc`/`-readme`/`-spec`) + §6 trigger-scope globs (source **and** derived side); we
  do not bootstrap.
- **Seed sources are uniform.** All **18** skills have `SKILL.md` + `README.md` + `SPEC.md`. No
  gaps; every page has the same three-source seed.
- **Toolchain present** (plan-035 `context.md`): d2, naba, pelican 4.11.0, uv, bd, git — no
  capability gate needed.

## Approach
Docs/web-infra + manifest change only; no skill `SPEC.md`/behavior change, so **no SPEC epic** and
no coverage-gate concern (mirrors plan-035). Three sequenced epics, keystone-first so the mechanism
exists before content, and drift wiring + guard-tighten + build/lint proof as the exit gate.

Design pillars:

1. **Invert the seam, keep the guarantees (Epic 1).** Rework `skill_pages.py` so an authored
   `web/content/skills/<name>.md` supplies the page **prose**, while the **"At a glance"** block,
   the `/skills/` index, and `SKILL_NAV` stay **generated from frontmatter**. The README
   auto-transform is **removed from the page body** (it is what authored prose replaces). During
   rollout the plugin **falls back** to the old README-transform for any skill lacking an authored
   page (so the site never breaks mid-migration); Epic 3 tightens this to a hard build error once
   all 18 pages exist. Update the plugin docstring; keep link-rewriting/anchor behavior intact.
2. **Rich authored pages, VOICE-governed (Epic 2).** Draft all 18 `web/content/skills/<name>.md`
   pages, each seeded richly from its `{SKILL.md,README.md,SPEC.md}` and rewritten to the public
   register per repo-root `VOICE.md` (plan-035). Parallelizable across skills; each page validated
   against Epic 1's new mechanism (renders, quick-ref intact, links resolve).
3. **Drift-enforce + prove (Epic 3).** Add the `skill-page` node, the three node-sourced
   `e-skill-page-*` edges (per-skill pairing) with their per-edge contract, and the §6 trigger-scope
   globs (source + derived side) to `DRIFT-CHECK.md`;
   tighten the plugin's missing-page guard to fail-closed; then the exit gate: full `yf-markdown-lint`
   audit over the new pages, a **0-warning Pelican build** with all 18 pages present and nav/index
   correct, and a `yf-drift-check` run over the new edge returning PASS.

### Composition detail (Epic 1 target)
The rendered page becomes, top to bottom: the skill title + summary (frontmatter), the auto
**"At a glance"** block (frontmatter), then the **authored prose** body (`web/content/skills/<name>.md`,
its own headings — typically *When it fires*, *How it works*, *Usage*, behavior notes). The
authored file MAY carry a YAML frontmatter block (stripped before render, as the current override
path already does). Precedence: **authored page present → use it (no README transform); absent →
fallback to README transform** (rollout only).
...
_No capability gate: d2/naba/pelican/uv/bd/git verified present (plan-035 `context.md`); no
environment prerequisite. No reconcile gate: no upstream issue is incorporated (`resolves-upstream`
is empty)._
