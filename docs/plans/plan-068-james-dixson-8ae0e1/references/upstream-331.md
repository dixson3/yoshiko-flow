...
type: Reference
okf_spec: OKF-PLAN
description: 'Upstream issue #331 - `land` is incompatible with `execute.worktree:
  false` — no execute branch is ever created'
---
# Upstream #331: `land` is incompatible with `execute.worktree: false` — no execute branch is ever created
