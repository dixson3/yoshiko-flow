...
type: Reference
okf_spec: OKF-PLAN
description: 'Upstream issue #352 - land --dry-run never checks that a draft body
  satisfies requires_mention, so the failure surfaces only after the writes are public'
---
# Upstream #352: land --dry-run never checks that a draft body satisfies requires_mention, so the failure surfaces only after the writes are public

- **Number:** 352
- **Title:** land --dry-run never checks that a draft body satisfies requires_mention, so the failure surfaces only after the writes are public
- **URL:** 
- **State:** OPEN
- **Labels:** 
...
<sub>Filed from plan `plan-063-james-dixson-3f74c1` (bundle: `docs/plans/plan-063-james-dixson-3f74c1`), recorded there as RE-004. Bead: `yf-t7t9`. Companion: the `LAND_DIGEST_EXCLUDED` issue filed alongside it — same origin, different cause.</sub>
