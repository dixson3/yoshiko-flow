...
type: Reference
okf_spec: OKF-PLAN
description: 'Upstream issue #353 - LAND_DIGEST_EXCLUDED omits resolved_target_tip
  and merge_preview, which L4/L6 self-mutate: every resume at or after L_VALIDATED
  is a guaranteed digest mismatch'
---
# Upstream #353: LAND_DIGEST_EXCLUDED omits resolved_target_tip and merge_preview, which L4/L6 self-mutate: every resume at or after L_VALIDATED is a guaranteed digest mismatch

- **Number:** 353
- **Title:** LAND_DIGEST_EXCLUDED omits resolved_target_tip and merge_preview, which L4/L6 self-mutate: every resume at or after L_VALIDATED is a guaranteed digest mismatch
- **URL:** 
- **State:** OPEN
- **Labels:** 
...
<sub>Filed from plan `plan-063-james-dixson-3f74c1` (bundle: `docs/plans/plan-063-james-dixson-3f74c1`), recorded there as RE-005. Bead: `yf-jp7z`. Companion: the `requires_mention` dry-run issue filed alongside it — same origin, different cause.</sub>
