...
- Issue 0.1: Fix the two live contradictions that FAIL under the CURRENT rule, so they do not pollute the new failure list: the lint-subset count (the web page's **seven** is correct per `yf-markdown-lint/SKILL.md:201`; **four** SKILL.mds say six), and `yf-incubator.md:61`'s "beads-free utility skill" against its own `skill-group: workflows` + `depends-on-skill: [yf-beads-extra]`.
...
- Issue 0.3: Add `## Invocation` to the 6 `user-invocable: true` skills that lack it — `yf-beads-hygiene`, `yf-beads-init`, `yf-beads-upstream`, `yf-diagram-authoring`, `yf-herdr`, `yf-research` — converting 6 structurally vacuous checks into live ones.
  - depends-on: 0.2
- Issue 0.4: Fix the `user-invocable` coercion defect, SPEC-first. `frontmatter.rs:61` types it `Option<bool>` (absent = unknown) while `skill_pages.py:133` coerces absent to `False`, so four markdown skills render as "auto (fires from its description conditions)" while their own descriptions read `TRIGGER when: /yf-markdown-lint invoked`. **This is a live false claim on the published site.** Either populate all 20 or make the reader fail-closed on absent.
- Issue 0.5: Amend the drift-engine spec for the required-set semantics and the new CLI→page direction — new/revised `REQ-*` ids, root `SPEC.md` amendment-log entry, and a tagged test in the same change-set, ahead of code.
  - depends-on: 0.2
...
- Issue 1.1: Add the omission check to `check_web_counts.py` — `missing = [s for s in truth if s not in members]` beside the existing `wrong` at `:179`. EXP-001 measured the current pipeline exiting **0** on a two-member deletion with the count left unchanged.
  - depends-on: 0.5
- Issue 1.2: Implement the slash-sub-verb required set as a mechanical realizer with a **vacuity floor**, so a shape change cannot silently zero it. Scope: every sub-verb a skill's normalised `## Invocation` declares must be named on its web page.
  - depends-on: 0.3, 0.5
- Issue 1.3: Add the **CLI→page direction** to `e-web-cli-surface` with a set-difference realizer (`check_skill_page_contract.py`'s shape — an exit code, not a prose edge). Catches `prune-private`, `--prune-formulas`, both `--force` flags.
  - depends-on: 0.5
...
### Epic 3: The layout re-pin
- Issue 3.1: Re-render all six existing diagrams under `--layout dagre`, unchanged sources, into a review set. Do NOT change the pin yet.
  - depends-on: 2.6
- Issue 3.2: Change `D2_FLAGS` in `plan066_checks.py`-equivalent, `yf-diagram-authoring`'s recipe, and any other pinned site, ONLY after the operator's read. Record in the commit that this invalidates every committed PNG's sha256 simultaneously — a one-shot expected mass diff, not drift.
  - depends-on: 3.1
...
- Issue 4.1: Rebuild `architecture.d2` as a layered marketecture — tool dependencies (`bd`, `gh`, `pandoc`, `d2`, `uv`, `git`, `xelatex`, `herdr`) at the bottom, beads + utility + markdown skills in the middle, workflow skills on top. Measured gaps to close: **5 of 8 declared tool deps absent**, **7 of 12 `yf` subcommand paths absent**, and **0 of 10 `depends-on-skill` edges drawn** — including the workflows→utility relationship #373 names.
  - depends-on: 3.2
- Issue 4.2: Combine `phase-model.d2` + `lifecycle.d2` into one, adding the red-team review **cycle**, capability gates during EXECUTE, escalations, retrospectives, autonomy tokens, and `capture`.
  - depends-on: 3.2
- Issue 4.3: Combine `install-matrix.d2` + `tune-matrix.d2`. Note `tune-matrix` measured **0 omissions** — it is correct and is being merged, not repaired. Preserve its `surface_dir` vs `skills_subpath` distinction, which is right and which a naive pass would break.
  - depends-on: 3.2
- Issue 4.4: Add the skills/agents → formulas map #373 asks for, alongside the existing `formulas.d2` (which measured 0 count/step omissions).
  - depends-on: 3.2
...
| R5 | The dagre re-pin invalidates six PNG sha256s at once and is misread as drift or as six failures. | med | Gated on an operator read (3.1 → gate → 3.2); the commit records it as an expected one-shot mass diff. |
...
| :-- | :-- | :-- | :-- |
| SC1 | The two live contradictions are fixed — the lint subset reads seven at every site, and `yf-incubator.md` no longer calls a `workflows` skill "beads-free utility" | `uv run scripts/checks/plan067_checks.py contradictions` → exit 0 | 0.1 |
...
| SC6 | `check_web_counts.py` FAILS on an omission — the two-member deletion that exits 0 today | `uv run scripts/checks/plan067_checks.py omission-fails` → exit 0 | 1.1 |
...
| SC8 | The CLI→page direction exists and catches `prune-private`, `--prune-formulas` and both `--force` flags | `uv run scripts/checks/plan067_checks.py cli-to-page` → exit 0 | 1.3 |
...
| SC10 | Every new checker was OBSERVED to fail against a code-side mutation | `uv run scripts/checks/test_negative_controls.py --all --min-checkers 4` → exit 0 | 1.5 |
...
| SC19 | `architecture.d2` shows the tool layer, the `yf` subcommand paths, and the `depends-on-skill` edges including workflows→utility | `uv run scripts/checks/plan067_checks.py architecture-complete` → exit 0 | 4.1 |
...
written. `0.7` (record the band) and `5.1` (factor out the shared model) are bookkeeping whose
effect is asserted by the criteria that execute against them; `5.3` adds an optional field whose
absence is legal for 17 of 20 skills; `6.5` is discharged by the reconcile gate and the §6.4
