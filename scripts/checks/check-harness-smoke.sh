...
#
# USAGE: check-harness-smoke.sh [verify-all|pi|opencode] [--transcript <path>]
#
# PROVENANCE: copied (never moved) from plan-054's bundle by plan-055 Issue 0.8. It lands
# DORMANT — plan-055 Issue 4.6 removes its only CHANGE-VALIDATION.md recipe row, and no issue
# in plan-055 rewrites it. The copy is pre-positioning for the deferred state-model follow-up
# (blocked on the change_validation exit-2 mapping fix), not a response to in-plan edits.
set -uo pipefail

CHECK_NAME=check-harness-smoke
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# TRANSCRIPT IS AN EXPLICIT ARGUMENT, NOT A PATH INFERENCE (plan-055 Issue 0.8 re-basing).
# plan-054's copy derived it from `${BASH_SOURCE[0]}/../../assets/` — correct at
# `<plan-dir>/assets/checks/`, meaningless at `scripts/checks/`, where there is no plan above
# this file. `--transcript <path>` wins; else `YF_SMOKE_TRANSCRIPT`; else a repo-local default
# under `.scratch/` (ignore-listed by REQ-YF-MARK-005), which is a scratch path rather than a
# guess at somebody's plan bundle.
MODE="verify-all"
TRANSCRIPT="${YF_SMOKE_TRANSCRIPT:-}"
while [ "$#" -gt 0 ]; do
  case "$1" in
...
if [ -z "${TRANSCRIPT}" ]; then
  _root="$(git -C "${HERE}" rev-parse --show-toplevel 2>/dev/null || printf '%s' "${HERE}")"
  mkdir -p "${_root}/.scratch" 2>/dev/null || true
  TRANSCRIPT="${_root}/.scratch/harness-smoke-transcript.md"
fi
